...
if ($exists) {echo '<script language="javascript"> window.location.href = "registrasi.php?desc=email-ready" </script>';break;}
...
try {
$sql = $db->prepare("INSERT INTO pelanggan (email, nama, alamat, no_wa, password, tgl_lahir) VALUES (? ,?, ?, ?, ?, ?)");
$sql->bindParam(1, $_POST["email"]);
$sql->bindParam(2, $_POST["nama"]);
$sql->bindParam(3, $_POST["alamat"]);
$sql->bindParam(4, $_POST["no_wa"]);
$sql->bindParam(5, $hash);
$sql->bindParam(6, $_POST["tgl_lahir"]);
$sql->execute();
